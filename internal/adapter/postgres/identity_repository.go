...
func (r *IdentityRepository) BindOrLoad(ctx context.Context, identityProvider, subject string) (identity.User, error) {
	tx, err := r.pool.Begin(ctx)
	if err != nil {
		return identity.User{}, fmt.Errorf("postgres: begin identity bind: %w", err)
	}
	defer tx.Rollback(ctx)

	lockKey := identityProvider + ":" + subject
	if _, err := tx.Exec(ctx, `SELECT pg_advisory_xact_lock(hashtextextended($1, 0))`, lockKey); err != nil {
		return identity.User{}, fmt.Errorf("postgres: lock identity bind: %w", err)
	}
	...
	if err == nil {
		return identity.User{ID: userID}, tx.Commit(ctx)
	}
	if !errors.Is(err, pgx.ErrNoRows) {
		return identity.User{}, fmt.Errorf("postgres: load identity binding: %w", err)
	}

	if err := tx.QueryRow(ctx, `
		INSERT INTO platform_users (id) VALUES (gen_random_uuid())
		RETURNING id::text`).Scan(&userID); err != nil {
		return identity.User{}, fmt.Errorf("postgres: insert platform user: %w", err)
	}
	...
	if _, err := tx.Exec(ctx, `
		...
		return identity.User{}, fmt.Errorf("postgres: insert identity binding: %w", err)
	}

	if err := tx.Commit(ctx); err != nil {
		return identity.User{}, fmt.Errorf("postgres: commit identity bind: %w", err)
	}
