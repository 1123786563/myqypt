# [T86.3] WeChat Pay 与 Alipay Merchant Dossier Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 确认微信支付和支付宝的商户资质、证书、回调、主动查询、退款、对账与生产验收要求。
...
**Spec:** [GitHub Issue #92](https://github.com/1123786563/myqypt/issues/92), `docs/architecture/external-confirmations.md`, `docs/architecture/architecture-baseline-risk-assessment-v1.1.md`, `docs/adr/0018-own-real-money-transactions-in-platform-commerce.md`, `docs/adr/0021-require-provider-verified-idempotent-payment-transitions.md`, `docs/adr/0042-reconcile-payment-hourly-and-usage-daily.md`
...
- Create `docs/evidence/dossiers/wechat-alipay-merchant/README.md` for the bounded question, scope, findings, contradictions, expiry, and paid-launch consequence.
- Create `docs/evidence/dossiers/wechat-alipay-merchant/sources.yaml` for machine-verifiable source metadata and evidence digests.
- Create `docs/evidence/dossiers/wechat-alipay-merchant/decision.yaml` for the accountable approve/block decision and renewal trigger.
- Create `tests/evidence/wechat_alipay_merchant_test.go` for schema, freshness, digest, and approval-state validation.

### Task 1: Produce one reviewable WeChat Pay 与 Alipay Merchant Dossier dossier

**Files:**
- Create: `docs/evidence/dossiers/wechat-alipay-merchant/README.md`
- Create: `docs/evidence/dossiers/wechat-alipay-merchant/sources.yaml`
- Create: `docs/evidence/dossiers/wechat-alipay-merchant/decision.yaml`
- Create: `tests/evidence/wechat_alipay_merchant_test.go`

**Interfaces:**
- Consumes: the exact external-confirmation question in Issue #92 and the relevant ADRs above.
...
func TestWechatAlipayMerchantDossierIsApproved(t *testing.T) {
    raw, err := os.ReadFile("../../docs/evidence/dossiers/wechat-alipay-merchant/decision.yaml")
    if err != nil {
        t.Fatal(err)
    }
    var got decision
...
Run: `go test ./tests/evidence -run TestWechatAlipayMerchantDossierIsApproved -count=1`

Expected: FAIL because `docs/evidence/dossiers/wechat-alipay-merchant/decision.yaml` does not exist or remains `blocked_pending_review`.

- [ ] **Step 3: Create the source manifest before collecting conclusions**

```yaml
dossier_id: wechat-alipay-merchant
issue: 92
...
Initialize `docs/evidence/dossiers/wechat-alipay-merchant/decision.yaml` with `status: blocked_pending_review`; this is a real blocking state, not an approval placeholder.

- [ ] **Step 4: Collect and cross-check the bounded evidence**

Record evidence sufficient to decide: **确认微信支付和支付宝的商户资质、证书、回调、主动查询、退款、对账与生产验收要求。** For every claim in `README.md`, cite a `source_id`; record contradictions and unanswered questions as explicit blockers with owner and next review date. Store references and digests rather than confidential source payloads.
...
```bash
git add docs/evidence/dossiers/wechat-alipay-merchant/README.md docs/evidence/dossiers/wechat-alipay-merchant/sources.yaml docs/evidence/dossiers/wechat-alipay-merchant/decision.yaml tests/evidence/wechat_alipay_merchant_test.go
git commit -m "docs(evidence): add wechat-alipay-merchant dossier"
```

Add the source-manifest digest, validation command, result, and decision status to Issue #92; do not paste confidential source content.
