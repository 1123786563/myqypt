# [T86.6] Mainland China Cloud Capability Dossier Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 确认选定云厂商在中国大陆 Region 的 KMS、Secret、PostgreSQL、Kafka、ClickHouse、Registry、多 AZ 与备份能力。
...
**Spec:** [GitHub Issue #95](https://github.com/1123786563/myqypt/issues/95), `docs/architecture/external-confirmations.md`, `docs/architecture/architecture-baseline-risk-assessment-v1.1.md`, `docs/adr/0005-launch-in-one-mainland-china-region.md`, `docs/adr/0043-use-one-mainland-region-across-multiple-availability-zones.md`, `docs/adr/0052-isolate-component-state-in-logical-postgresql-databases.md`
...
- Create `docs/evidence/dossiers/mainland-cloud-capabilities/README.md` for the bounded question, scope, findings, contradictions, expiry, and paid-launch consequence.
- Create `docs/evidence/dossiers/mainland-cloud-capabilities/sources.yaml` for machine-verifiable source metadata and evidence digests.
- Create `docs/evidence/dossiers/mainland-cloud-capabilities/decision.yaml` for the accountable approve/block decision and renewal trigger.
- Create `tests/evidence/mainland_cloud_capabilities_test.go` for schema, freshness, digest, and approval-state validation.

### Task 1: Produce one reviewable Mainland China Cloud Capability Dossier dossier

**Files:**
- Create: `docs/evidence/dossiers/mainland-cloud-capabilities/README.md`
- Create: `docs/evidence/dossiers/mainland-cloud-capabilities/sources.yaml`
- Create: `docs/evidence/dossiers/mainland-cloud-capabilities/decision.yaml`
- Create: `tests/evidence/mainland_cloud_capabilities_test.go`

**Interfaces:**
- Consumes: the exact external-confirmation question in Issue #95 and the relevant ADRs above.
...
func TestMainlandCloudCapabilitiesDossierIsApproved(t *testing.T) {
    raw, err := os.ReadFile("../../docs/evidence/dossiers/mainland-cloud-capabilities/decision.yaml")
    if err != nil {
        t.Fatal(err)
    }
    var got decision
...
Run: `go test ./tests/evidence -run TestMainlandCloudCapabilitiesDossierIsApproved -count=1`

Expected: FAIL because `docs/evidence/dossiers/mainland-cloud-capabilities/decision.yaml` does not exist or remains `blocked_pending_review`.

- [ ] **Step 3: Create the source manifest before collecting conclusions**

```yaml
dossier_id: mainland-cloud-capabilities
issue: 95
...
Initialize `docs/evidence/dossiers/mainland-cloud-capabilities/decision.yaml` with `status: blocked_pending_review`; this is a real blocking state, not an approval placeholder.

- [ ] **Step 4: Collect and cross-check the bounded evidence**

Record evidence sufficient to decide: **确认选定云厂商在中国大陆 Region 的 KMS、Secret、PostgreSQL、Kafka、ClickHouse、Registry、多 AZ 与备份能力。** For every claim in `README.md`, cite a `source_id`; record contradictions and unanswered questions as explicit blockers with owner and next review date. Store references and digests rather than confidential source payloads.
...
```bash
git add docs/evidence/dossiers/mainland-cloud-capabilities/README.md docs/evidence/dossiers/mainland-cloud-capabilities/sources.yaml docs/evidence/dossiers/mainland-cloud-capabilities/decision.yaml tests/evidence/mainland_cloud_capabilities_test.go
git commit -m "docs(evidence): add mainland-cloud-capabilities dossier"
```

Add the source-manifest digest, validation command, result, and decision status to Issue #95; do not paste confidential source content.
