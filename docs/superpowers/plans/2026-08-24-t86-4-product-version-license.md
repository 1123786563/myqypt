# [T86.4] Product Version License Dossier Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 汇总每个 Product Version 的 License Report、商业托管权与法律批准记录。
...
**Spec:** [GitHub Issue #93](https://github.com/1123786563/myqypt/issues/93), `docs/architecture/external-confirmations.md`, `docs/architecture/architecture-baseline-risk-assessment-v1.1.md`, `docs/adr/0039-run-license-gates-for-every-product-version.md`, `docs/adr/0040-use-a-managed-private-registry-before-harbor.md`
...
- Create `docs/evidence/dossiers/product-version-license/README.md` for the bounded question, scope, findings, contradictions, expiry, and paid-launch consequence.
- Create `docs/evidence/dossiers/product-version-license/sources.yaml` for machine-verifiable source metadata and evidence digests.
- Create `docs/evidence/dossiers/product-version-license/decision.yaml` for the accountable approve/block decision and renewal trigger.
- Create `tests/evidence/product_version_license_test.go` for schema, freshness, digest, and approval-state validation.

### Task 1: Produce one reviewable Product Version License Dossier dossier

**Files:**
- Create: `docs/evidence/dossiers/product-version-license/README.md`
- Create: `docs/evidence/dossiers/product-version-license/sources.yaml`
- Create: `docs/evidence/dossiers/product-version-license/decision.yaml`
- Create: `tests/evidence/product_version_license_test.go`

**Interfaces:**
- Consumes: the exact external-confirmation question in Issue #93 and the relevant ADRs above.
...
func TestProductVersionLicenseDossierIsApproved(t *testing.T) {
    raw, err := os.ReadFile("../../docs/evidence/dossiers/product-version-license/decision.yaml")
    if err != nil {
        t.Fatal(err)
    }
    var got decision
...
Run: `go test ./tests/evidence -run TestProductVersionLicenseDossierIsApproved -count=1`

Expected: FAIL because `docs/evidence/dossiers/product-version-license/decision.yaml` does not exist or remains `blocked_pending_review`.

- [ ] **Step 3: Create the source manifest before collecting conclusions**

```yaml
dossier_id: product-version-license
issue: 93
...
Initialize `docs/evidence/dossiers/product-version-license/decision.yaml` with `status: blocked_pending_review`; this is a real blocking state, not an approval placeholder.

- [ ] **Step 4: Collect and cross-check the bounded evidence**

Record evidence sufficient to decide: **汇总每个 Product Version 的 License Report、商业托管权与法律批准记录。** For every claim in `README.md`, cite a `source_id`; record contradictions and unanswered questions as explicit blockers with owner and next review date. Store references and digests rather than confidential source payloads.
...
```bash
git add docs/evidence/dossiers/product-version-license/README.md docs/evidence/dossiers/product-version-license/sources.yaml docs/evidence/dossiers/product-version-license/decision.yaml tests/evidence/product_version_license_test.go
git commit -m "docs(evidence): add product-version-license dossier"
```

Add the source-manifest digest, validation command, result, and decision status to Issue #93; do not paste confidential source content.
