# [T86.7] Nacos Version 与 Production Baseline Dossier Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 在实施时重新确认 Nacos 稳定版本、Java 要求、鉴权、三节点、外置 PostgreSQL、备份与恢复边界。
...
**Spec:** [GitHub Issue #96](https://github.com/1123786563/myqypt/issues/96), `docs/architecture/external-confirmations.md`, `docs/architecture/architecture-baseline-risk-assessment-v1.1.md`, `docs/adr/0051-include-nacos-in-the-day-1-platform.md`, `docs/adr/0052-isolate-component-state-in-logical-postgresql-databases.md`
...
- Create `docs/evidence/dossiers/nacos-production-baseline/README.md` for the bounded question, scope, findings, contradictions, expiry, and paid-launch consequence.
- Create `docs/evidence/dossiers/nacos-production-baseline/sources.yaml` for machine-verifiable source metadata and evidence digests.
- Create `docs/evidence/dossiers/nacos-production-baseline/decision.yaml` for the accountable approve/block decision and renewal trigger.
- Create `tests/evidence/nacos_production_baseline_test.go` for schema, freshness, digest, and approval-state validation.

### Task 1: Produce one reviewable Nacos Version 与 Production Baseline Dossier dossier

**Files:**
- Create: `docs/evidence/dossiers/nacos-production-baseline/README.md`
- Create: `docs/evidence/dossiers/nacos-production-baseline/sources.yaml`
- Create: `docs/evidence/dossiers/nacos-production-baseline/decision.yaml`
- Create: `tests/evidence/nacos_production_baseline_test.go`

**Interfaces:**
- Consumes: the exact external-confirmation question in Issue #96 and the relevant ADRs above.
...
func TestNacosProductionBaselineDossierIsApproved(t *testing.T) {
    raw, err := os.ReadFile("../../docs/evidence/dossiers/nacos-production-baseline/decision.yaml")
    if err != nil {
        t.Fatal(err)
    }
    var got decision
...
Run: `go test ./tests/evidence -run TestNacosProductionBaselineDossierIsApproved -count=1`

Expected: FAIL because `docs/evidence/dossiers/nacos-production-baseline/decision.yaml` does not exist or remains `blocked_pending_review`.

- [ ] **Step 3: Create the source manifest before collecting conclusions**

```yaml
dossier_id: nacos-production-baseline
issue: 96
...
Initialize `docs/evidence/dossiers/nacos-production-baseline/decision.yaml` with `status: blocked_pending_review`; this is a real blocking state, not an approval placeholder.

- [ ] **Step 4: Collect and cross-check the bounded evidence**

Record evidence sufficient to decide: **在实施时重新确认 Nacos 稳定版本、Java 要求、鉴权、三节点、外置 PostgreSQL、备份与恢复边界。** For every claim in `README.md`, cite a `source_id`; record contradictions and unanswered questions as explicit blockers with owner and next review date. Store references and digests rather than confidential source payloads.
...
```bash
git add docs/evidence/dossiers/nacos-production-baseline/README.md docs/evidence/dossiers/nacos-production-baseline/sources.yaml docs/evidence/dossiers/nacos-production-baseline/decision.yaml tests/evidence/nacos_production_baseline_test.go
git commit -m "docs(evidence): add nacos-production-baseline dossier"
```

Add the source-manifest digest, validation command, result, and decision status to Issue #96; do not paste confidential source content.
