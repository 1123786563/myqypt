# [T86.8] Valkey 与 OpenMeter Compatibility Dossier Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 收集 Valkey 对当前 OpenMeter Redis 命令面、TLS、持久化、并发去重、Failover 与恢复的可复现证据。
...
**Spec:** [GitHub Issue #97](https://github.com/1123786563/myqypt/issues/97), `docs/architecture/external-confirmations.md`, `docs/architecture/architecture-baseline-risk-assessment-v1.1.md`, `docs/adr/0027-use-valkey-for-persistent-deduplication.md`
...
- Create `docs/evidence/dossiers/valkey-openmeter-compatibility/README.md` for the bounded question, scope, findings, contradictions, expiry, and paid-launch consequence.
- Create `docs/evidence/dossiers/valkey-openmeter-compatibility/sources.yaml` for machine-verifiable source metadata and evidence digests.
- Create `docs/evidence/dossiers/valkey-openmeter-compatibility/decision.yaml` for the accountable approve/block decision and renewal trigger.
- Create `tests/evidence/valkey_openmeter_compatibility_test.go` for schema, freshness, digest, and approval-state validation.

### Task 1: Produce one reviewable Valkey 与 OpenMeter Compatibility Dossier dossier

**Files:**
- Create: `docs/evidence/dossiers/valkey-openmeter-compatibility/README.md`
- Create: `docs/evidence/dossiers/valkey-openmeter-compatibility/sources.yaml`
- Create: `docs/evidence/dossiers/valkey-openmeter-compatibility/decision.yaml`
- Create: `tests/evidence/valkey_openmeter_compatibility_test.go`

**Interfaces:**
- Consumes: the exact external-confirmation question in Issue #97 and the relevant ADRs above.
...
func TestValkeyOpenmeterCompatibilityDossierIsApproved(t *testing.T) {
    raw, err := os.ReadFile("../../docs/evidence/dossiers/valkey-openmeter-compatibility/decision.yaml")
    if err != nil {
        t.Fatal(err)
    }
    var got decision
...
Run: `go test ./tests/evidence -run TestValkeyOpenmeterCompatibilityDossierIsApproved -count=1`

Expected: FAIL because `docs/evidence/dossiers/valkey-openmeter-compatibility/decision.yaml` does not exist or remains `blocked_pending_review`.

- [ ] **Step 3: Create the source manifest before collecting conclusions**

```yaml
dossier_id: valkey-openmeter-compatibility
issue: 97
...
Initialize `docs/evidence/dossiers/valkey-openmeter-compatibility/decision.yaml` with `status: blocked_pending_review`; this is a real blocking state, not an approval placeholder.

- [ ] **Step 4: Collect and cross-check the bounded evidence**

Record evidence sufficient to decide: **收集 Valkey 对当前 OpenMeter Redis 命令面、TLS、持久化、并发去重、Failover 与恢复的可复现证据。** For every claim in `README.md`, cite a `source_id`; record contradictions and unanswered questions as explicit blockers with owner and next review date. Store references and digests rather than confidential source payloads.
...
```bash
git add docs/evidence/dossiers/valkey-openmeter-compatibility/README.md docs/evidence/dossiers/valkey-openmeter-compatibility/sources.yaml docs/evidence/dossiers/valkey-openmeter-compatibility/decision.yaml tests/evidence/valkey_openmeter_compatibility_test.go
git commit -m "docs(evidence): add valkey-openmeter-compatibility dossier"
```

Add the source-manifest digest, validation command, result, and decision status to Issue #97; do not paste confidential source content.
