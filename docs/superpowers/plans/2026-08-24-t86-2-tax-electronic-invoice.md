# [T86.2] Tax 与 Electronic Invoice Dossier Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 由可承担责任的财税审核者确认税率、发票类型、开具时点、作废、红冲与法定保存期。
...
**Spec:** [GitHub Issue #91](https://github.com/1123786563/myqypt/issues/91), `docs/architecture/external-confirmations.md`, `docs/architecture/architecture-baseline-risk-assessment-v1.1.md`, `docs/adr/0054-support-electronic-invoice-requests-in-stage-1.md`
...
- Create `docs/evidence/dossiers/tax-electronic-invoice/README.md` for the bounded question, scope, findings, contradictions, expiry, and paid-launch consequence.
- Create `docs/evidence/dossiers/tax-electronic-invoice/sources.yaml` for machine-verifiable source metadata and evidence digests.
- Create `docs/evidence/dossiers/tax-electronic-invoice/decision.yaml` for the accountable approve/block decision and renewal trigger.
- Create `tests/evidence/tax_electronic_invoice_test.go` for schema, freshness, digest, and approval-state validation.

### Task 1: Produce one reviewable Tax 与 Electronic Invoice Dossier dossier

**Files:**
- Create: `docs/evidence/dossiers/tax-electronic-invoice/README.md`
- Create: `docs/evidence/dossiers/tax-electronic-invoice/sources.yaml`
- Create: `docs/evidence/dossiers/tax-electronic-invoice/decision.yaml`
- Create: `tests/evidence/tax_electronic_invoice_test.go`

**Interfaces:**
- Consumes: the exact external-confirmation question in Issue #91 and the relevant ADRs above.
...
func TestTaxElectronicInvoiceDossierIsApproved(t *testing.T) {
    raw, err := os.ReadFile("../../docs/evidence/dossiers/tax-electronic-invoice/decision.yaml")
    if err != nil {
        t.Fatal(err)
    }
    var got decision
...
Run: `go test ./tests/evidence -run TestTaxElectronicInvoiceDossierIsApproved -count=1`

Expected: FAIL because `docs/evidence/dossiers/tax-electronic-invoice/decision.yaml` does not exist or remains `blocked_pending_review`.

- [ ] **Step 3: Create the source manifest before collecting conclusions**

```yaml
dossier_id: tax-electronic-invoice
issue: 91
...
Initialize `docs/evidence/dossiers/tax-electronic-invoice/decision.yaml` with `status: blocked_pending_review`; this is a real blocking state, not an approval placeholder.

- [ ] **Step 4: Collect and cross-check the bounded evidence**

Record evidence sufficient to decide: **由可承担责任的财税审核者确认税率、发票类型、开具时点、作废、红冲与法定保存期。** For every claim in `README.md`, cite a `source_id`; record contradictions and unanswered questions as explicit blockers with owner and next review date. Store references and digests rather than confidential source payloads.
...
```bash
git add docs/evidence/dossiers/tax-electronic-invoice/README.md docs/evidence/dossiers/tax-electronic-invoice/sources.yaml docs/evidence/dossiers/tax-electronic-invoice/decision.yaml tests/evidence/tax_electronic_invoice_test.go
git commit -m "docs(evidence): add tax-electronic-invoice dossier"
```

Add the source-manifest digest, validation command, result, and decision status to Issue #91; do not paste confidential source content.
