# [T86.5] Model Provider Processing Terms Dossier Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 确认每个获批模型 Provider 的 Region、Retention、Training Use、Subprocessor 与商业 API 权利。
...
**Spec:** [GitHub Issue #94](https://github.com/1123786563/myqypt/issues/94), `docs/architecture/external-confirmations.md`, `docs/architecture/architecture-baseline-risk-assessment-v1.1.md`, `docs/adr/0045-use-platform-funded-models-only-through-higress.md`, `docs/adr/0047-bind-model-routing-to-data-processing-profiles.md`
...
- Create `docs/evidence/dossiers/model-provider-terms/README.md` for the bounded question, scope, findings, contradictions, expiry, and paid-launch consequence.
- Create `docs/evidence/dossiers/model-provider-terms/sources.yaml` for machine-verifiable source metadata and evidence digests.
- Create `docs/evidence/dossiers/model-provider-terms/decision.yaml` for the accountable approve/block decision and renewal trigger.
- Create `tests/evidence/model_provider_terms_test.go` for schema, freshness, digest, and approval-state validation.

### Task 1: Produce one reviewable Model Provider Processing Terms Dossier dossier

**Files:**
- Create: `docs/evidence/dossiers/model-provider-terms/README.md`
- Create: `docs/evidence/dossiers/model-provider-terms/sources.yaml`
- Create: `docs/evidence/dossiers/model-provider-terms/decision.yaml`
- Create: `tests/evidence/model_provider_terms_test.go`

**Interfaces:**
- Consumes: the exact external-confirmation question in Issue #94 and the relevant ADRs above.
...
func TestModelProviderTermsDossierIsApproved(t *testing.T) {
    raw, err := os.ReadFile("../../docs/evidence/dossiers/model-provider-terms/decision.yaml")
    if err != nil {
        t.Fatal(err)
    }
    var got decision
...
Run: `go test ./tests/evidence -run TestModelProviderTermsDossierIsApproved -count=1`

Expected: FAIL because `docs/evidence/dossiers/model-provider-terms/decision.yaml` does not exist or remains `blocked_pending_review`.

- [ ] **Step 3: Create the source manifest before collecting conclusions**

```yaml
dossier_id: model-provider-terms
issue: 94
...
Initialize `docs/evidence/dossiers/model-provider-terms/decision.yaml` with `status: blocked_pending_review`; this is a real blocking state, not an approval placeholder.

- [ ] **Step 4: Collect and cross-check the bounded evidence**

Record evidence sufficient to decide: **确认每个获批模型 Provider 的 Region、Retention、Training Use、Subprocessor 与商业 API 权利。** For every claim in `README.md`, cite a `source_id`; record contradictions and unanswered questions as explicit blockers with owner and next review date. Store references and digests rather than confidential source payloads.
...
```bash
git add docs/evidence/dossiers/model-provider-terms/README.md docs/evidence/dossiers/model-provider-terms/sources.yaml docs/evidence/dossiers/model-provider-terms/decision.yaml tests/evidence/model_provider_terms_test.go
git commit -m "docs(evidence): add model-provider-terms dossier"
```

Add the source-manifest digest, validation command, result, and decision status to Issue #94; do not paste confidential source content.
