# [T86.1] Privacy、未成年人、Export、Erasure 与 Operator Access Dossier Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 确认中国大陆隐私、未成年人保护、数据删除、Tenant Export、Operator Access、事件通知与法定保留边界。
...
**Spec:** [GitHub Issue #90](https://github.com/1123786563/myqypt/issues/90), `docs/architecture/external-confirmations.md`, `docs/architecture/architecture-baseline-risk-assessment-v1.1.md`, `docs/adr/0005-launch-in-one-mainland-china-region.md`, `docs/adr/0011-erase-product-data-after-read-only-retention.md`, `docs/adr/0048-use-consented-jit-operator-access.md`, `docs/adr/0050-require-verifiable-tenant-export-from-shared-products.md`
...
- Create `docs/evidence/dossiers/privacy-data-governance/README.md` for the bounded question, scope, findings, contradictions, expiry, and paid-launch consequence.
- Create `docs/evidence/dossiers/privacy-data-governance/sources.yaml` for machine-verifiable source metadata and evidence digests.
- Create `docs/evidence/dossiers/privacy-data-governance/decision.yaml` for the accountable approve/block decision and renewal trigger.
- Create `tests/evidence/privacy_data_governance_test.go` for schema, freshness, digest, and approval-state validation.

### Task 1: Produce one reviewable Privacy、未成年人、Export、Erasure 与 Operator Access Dossier dossier

**Files:**
- Create: `docs/evidence/dossiers/privacy-data-governance/README.md`
- Create: `docs/evidence/dossiers/privacy-data-governance/sources.yaml`
- Create: `docs/evidence/dossiers/privacy-data-governance/decision.yaml`
- Create: `tests/evidence/privacy_data_governance_test.go`

**Interfaces:**
- Consumes: the exact external-confirmation question in Issue #90 and the relevant ADRs above.
...
func TestPrivacyDataGovernanceDossierIsApproved(t *testing.T) {
    raw, err := os.ReadFile("../../docs/evidence/dossiers/privacy-data-governance/decision.yaml")
    if err != nil {
        t.Fatal(err)
    }
    var got decision
...
Run: `go test ./tests/evidence -run TestPrivacyDataGovernanceDossierIsApproved -count=1`

Expected: FAIL because `docs/evidence/dossiers/privacy-data-governance/decision.yaml` does not exist or remains `blocked_pending_review`.

- [ ] **Step 3: Create the source manifest before collecting conclusions**

```yaml
dossier_id: privacy-data-governance
issue: 90
...
Initialize `docs/evidence/dossiers/privacy-data-governance/decision.yaml` with `status: blocked_pending_review`; this is a real blocking state, not an approval placeholder.

- [ ] **Step 4: Collect and cross-check the bounded evidence**

Record evidence sufficient to decide: **确认中国大陆隐私、未成年人保护、数据删除、Tenant Export、Operator Access、事件通知与法定保留边界。** For every claim in `README.md`, cite a `source_id`; record contradictions and unanswered questions as explicit blockers with owner and next review date. Store references and digests rather than confidential source payloads.
...
```bash
git add docs/evidence/dossiers/privacy-data-governance/README.md docs/evidence/dossiers/privacy-data-governance/sources.yaml docs/evidence/dossiers/privacy-data-governance/decision.yaml tests/evidence/privacy_data_governance_test.go
git commit -m "docs(evidence): add privacy-data-governance dossier"
```

Add the source-manifest digest, validation command, result, and decision status to Issue #90; do not paste confidential source content.
